        ...
        <div ref={rankingParent} className="ranking-list space-y-5 pt-1">
          {visibleGroups.map(group => <section key={group.key} className="ranking-placement-group min-w-0" data-placement={group.placement || undefined}>
            <header className="ranking-placement-header mb-2.5 flex min-w-0 items-center gap-3">
              {group.placement ? <span aria-label={`${group.placement}º lugar`} className={`ranking-placement-emblem flex size-12 shrink-0 items-center justify-center rounded-full border text-sm font-black shadow-lg ${group.placement === 1 ? 'ranking-medal ranking-medal-1 bg-gradient-to-br from-amber-200 to-amber-600 text-amber-950' : group.placement === 2 ? 'ranking-medal ranking-medal-2 bg-gradient-to-br from-zinc-100 to-zinc-400 text-zinc-800' : group.placement === 3 ? 'ranking-medal ranking-medal-3 bg-gradient-to-br from-amber-500 to-amber-900 text-white' : 'ranking-position border-white/10 bg-zinc-800 text-zinc-200'}`}><span className="ranking-planet-surface" aria-hidden="true" /><span className="ranking-placement-label">{group.placement}º</span></span> : <span className="ranking-date-label inline-flex h-10 shrink-0 items-center gap-2 rounded-full border border-white/10 bg-white/[.035] px-3 text-[11px] font-extrabold text-zinc-300"><CalendarPlus className="size-3.5" />{group.label}</span>}
