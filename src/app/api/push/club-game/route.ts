...
import { formatPushMonth, isAdminUser, isPushConfigured, sendPushNotification } from '@/lib/push';
...
export async function POST(request: Request) {
  ...
  const claim = await admin.from('push_notification_deliveries').insert({ event_key: `club-game:${month}:${gameId}`, user_id: user.id });
  if (claim.error) {
    if (claim.error.code === '23505') return NextResponse.json({ sent: 0, changed: false });
