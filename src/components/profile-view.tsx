...
export function ProfileView({ profileId, own = false }: { profileId: string; own?: boolean }) {
  ...
  async function saveProfile() {
    ...
    const patch = { name: name.trim() || data.profile.name || 'Membro', bio: bio.trim() || null, avatar_url: avatarUrl, avatar_crop: avatarUrl === data.profile.avatar_url ? data.profile.avatar_crop ?? null : null };
  ...
  return (
    <div className="mx-auto max-w-3xl animate-fade-in">
      <section className="profile-hero relative mb-7 overflow-hidden rounded-[30px] border border-white/8 bg-white/[0.025] p-6 text-center">
        ...
        <div className="profile-avatar-wrap relative mx-auto w-fit p-2"><Avatar src={person.avatar_url} crop={person.avatar_crop} name={person.name} className="size-24 border-2 border-violet-400/25 text-xl shadow-xl" /></div>
